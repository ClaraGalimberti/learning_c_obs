...
from losses.losses import MultiStepLoss
...
loss_log_4 = torch.zeros(p.epochs)
...
loss_4 = LocalOptLoss(sys, sys_z, coup, model_Px, p.r)
...
for epoch in range(p.epochs):
    ...
    loss2 = loss_2(coup.delift(coup.forward_inverse(coup.forward(coup.lift(x_data)))), x_data)
    loss3 = loss_3(x_data) * 100
    loss4 = loss_4(x_data, e_batch)
    # loss4 = compute_l3_batch(x_data, e_batch, model_Px, coup.tau, coup.sigma, sys.dynamics, sys.output, sys_z, R)
    loss = loss1 + loss2 + loss4
    loss_log_1[epoch] = loss1.detach()
    loss_log_2[epoch] = loss2.detach()
    loss_log_3[epoch] = loss3.detach()
    loss_log_4[epoch] = loss4.detach()
    if epoch % 10 == 0:
        msg =  "Epoch: %4i \t--- " % epoch
        msg += "Loss: %12.6f \t---||--- " % loss
        msg += "Loss 1: %12.6f \t---- " % loss1
        msg += "Loss 2: %12.6f \t---- " % loss2
        msg += "Loss 4: %12.6f \t---- " % loss4
        msg += "Loss steps: %8.6f" % loss3
        logger.info(msg)
    loss.backward()
    optimizer.step()
    scheduler.step()
