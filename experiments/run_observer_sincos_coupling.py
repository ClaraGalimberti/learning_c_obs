...
# tau = FCNN(dim_in=ny, dim_out=sys.state_dim, dim_hidden=nq*40)
# sigma = FCNN(dim_in=sys.state_dim, dim_out=ny, dim_hidden=nq*40)
...
optimizer = torch.optim.Adam(list(sys_z.parameters()) + list(coup.parameters()), lr=learning_rate)
...
for epoch in range(epochs):
    ...
    loss1 = loss_1(torch.bmm(sys.dynamics(x_data), jacobian), sys_z(t, coup(coup.lift(x_data)), sys.output(x_data)))
...
x_hat_log = coup.delift(coup.forward_inverse(xi_log))
plt.figure()
plt.subplot(2,1,1)
...
plt.legend()
plt.savefig(folder + "sincos_states_after_training.pdf", format='pdf')
plt.show()

plt.figure()
plt.plot(torch.linspace(0, epochs-1, epochs), log_loss_1, label=r'$\ell_1$')
plt.plot(torch.linspace(0, epochs-1, epochs), log_loss_2, label=r'$\ell_2$')
plt.legend()
ax = plt.gca()
ax.set_yscale('log')
plt.savefig(folder + "sincos_loss.pdf", format='pdf')
plt.show()

# x_hat_log_noisy:
xi_init = torch.randn(1,1,sys_z.nx)*2
xi_log_noisy = sys_z.rollout(xi_init, y_log_noisy, T)
x_hat_log_noisy = coup.delift(coup.forward_inverse(xi_log_noisy))
plt.figure()
plt.subplot(3,1,1)
...
plt.legend()
plt.savefig(folder + "sincos_noisy_states_after_training.pdf", format='pdf')
plt.show()
